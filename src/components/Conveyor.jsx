  ...
  return (
    <section className="relative bg-bone py-24 md:py-32 overflow-hidden border-y border-ink-100">
      ...
      <div className="container-page mt-16 md:mt-20 grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-10">
        {t.conveyor.pillars.map((s, i) => (
          <div key={s.kpi} className="reveal" style={{ transitionDelay: `${i * 80}ms` }}>
            <div className="font-display text-2xl md:text-3xl font-semibold text-ink-900 tracking-ultra-tight mb-1.5">
              {s.kpi}
            </div>
            <div className="text-[12px] md:text-[13px] text-ink-500">{s.sub}</div>
          </div>
        ))}
      </div>
    </section>
    ...
    <div className={`relative h-20 md:h-24 ${className}`}>
      ...
      >
        {items.map((item) => {
          ...
          return (
            <div key={item.key} className={`${size} flex-shrink-0`} style={{ transform: `scale(${item.scale})` }}>
...
function Belt({ offset = '50%' }) {
  return (
    <div className="absolute left-0 right-0 h-px bg-ink-200 pointer-events-none" style={{ top: offset }} aria-hidden>
      <div
        className="absolute top-0 left-0 right-0 h-px"
        style={{
          backgroundImage: 'linear-gradient(to right, transparent, rgba(232, 93, 31, 0.6), transparent)',
